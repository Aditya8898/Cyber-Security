...
const attachCounts = async (posts) => {
...
export const getAllBlogPosts = async (req, res) => {
    try {
        ...
        const finalPosts = await attachCounts(posts);
...
export const toggleLike = async (req, res) => {
    try {
        ...
        if (post.status !== "approved") {
            return res.status(403).json({
                success: false,
                message: "Only approved posts can be liked",
            });
        }
...
// ADMIN: GET ALL POSTS WITH STATUS (optional ?status=pending|approved|rejected)
